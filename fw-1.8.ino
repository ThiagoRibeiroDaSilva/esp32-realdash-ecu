...
// Firmware V1.8 | Dossie V21.0 (The Software Bridge)
// Framework: Arduino IDE | FreeRTOS Dual-Core
//
// CHANGELOG V1.8 (Termodinâmica do Arrefecimento):
//   1. ADICIONADO: Offset de compensação de temperatura (+7°C padrão) para
//      leitura na bomba d'água refletir a temperatura real do bloco.
//   2. ADICIONADO: Filtro Assimétrico de Temperatura (Gauge Deadzone).
//      - Se a temp SOBE: Atualiza instantaneamente (modo pânico).
//      - Se a temp CAI: Aplica média exponencial de 5% (suaviza a abertura da termostática).
//   3. ADICIONADO: Fase de Aquecimento (Warm-up) na interface gráfica.
//      Temperaturas abaixo de 65°C exibem "FRIO" no TFT em vez de números flutuantes.
//
...
//   4. ADICIONADO: Filtro de glitch por hardware no GPIO de RPM
//   5. CORRIGIDO: Diagnóstico ADS1115 agora lê registrador real via I2C
...
#define PIN_I2C_SDA       8    // GPIO 8  — I2C SDA confirmado no netlist
#define PIN_I2C_SCL       9    // GPIO 9  — I2C SCL confirmado no netlist
...
// =============================================================================
// SEÇÃO 2: CONSTANTES DE HARDWARE & FILTROS (Dossiê V21.0)
// =============================================================================
...
#define NTC_R0              2200.0f     // Resistência nominal a T0
#define NTC_T0_KELVIN       298.15f     // T0 = 25°C em Kelvin
#define NTC_BETA            3950.0f     // Coeficiente Beta do MTE4053

// Termodinâmica e Filtro Assimétrico da Água (Novo V1.8)
#define WATER_TEMP_OFFSET   7.0f        // Compensação: +7°C para sensor na bomba refletir o bloco
#define WATER_TEMP_WARMUP   65.0f       // Abaixo dessa temp, mostra "FRIO" no painel
#define WATER_FILTER_ALPHA  0.05f       // Média exponencial quando esfria (5% novo, 95% velho)
...
#define ADS_RECHECK_MS      1000UL      
#define GPS_VALID_TIMEOUT_MS 2000UL     
...
#define REALDASH_FRAME_ID_1 0x001   
#define REALDASH_FRAME_ID_2 0x002   
...
// Função alterada: Agora retorna o valor físico puro (Raw).
// O filtro e o offset são aplicados no backend para preservar o State.
...
static FuelStatus_t calcFuelRatio(float v_boia, float v_feed, float* result) {
    *result = 0.0f;
    ...
    float ratio = v_boia / v_feed;
    if (isnan(ratio) || isinf(ratio) || ratio < 0.0f) return FUEL_ERR_RANGE;
    if (ratio > 1.15f) return FUEL_ERR_RANGE; 
...
static bool adsApplyRuntimeConfig() {
    ...
    uint16_t cfg = 0;
    return adsReadRegister16(0x01, &cfg);
}
...
static void taskBackend(void* pvParam) {
    ...
    static float          sLastWaterTemp = 0.0f;  // Agora guarda o valor FILTRADO
    ...
    for (;;) {
        ...
        if (adsOk) {
            ...
            // 3. Temperatura da Água (Com Offset e Filtro Assimétrico V1.8)
            float v_a1 = adsReadVolts(1, GAIN_ONE);
            float raw_water = 0.0f;
            water_st = calcWaterTemp(v_a1, v_alim, &raw_water);

            if (water_st == SENSOR_OK) {
                // Aplica a calibração física da diferença bomba vs bloco
                float tempReal = raw_water + WATER_TEMP_OFFSET;
                
                // Primeira leitura válida: crava a temperatura sem filtro
                if (sLastWaterTemp == 0.0f || sLastWaterTemp < -20.0f) {
                    sLastWaterTemp = tempReal;
                } else {
                    // O Filtro Assimétrico Mágico
                    if (tempReal >= sLastWaterTemp) {
                        // Esquentou: Atualiza IMEDIATAMENTE (modo pânico ativado)
                        sLastWaterTemp = tempReal;
                    } else {
                        // Esfriou (termostática abriu ou ventoinha ligou): 
                        // Média exponencial super lenta para o painel não despencar
                        sLastWaterTemp = (tempReal * WATER_FILTER_ALPHA) + 
                                         (sLastWaterTemp * (1.0f - WATER_FILTER_ALPHA));
                    }
                }
                water_temp = sLastWaterTemp; 
            } else {
                // Em caso de curto/falha, repassa o código de erro numérico puro
                water_temp = raw_water; 
            }
            ...
            sLastWaterTemp = water_temp; // Já salva o filtrado pro próximo loop
        ...
        if (xSemaphoreTake(gDataMutex, pdMS_TO_TICKS(10))) {
            ...
            gData.water_temp_c  = water_temp; // Agora recebe o valor já filtrado e com offset
...
static void taskFrontend(void* pvParam) {
    ...
    gTFT.setCursor(100,150); gTFT.print("CHEVETTE ECU  V1.8");
    ...
    for (;;) {
        ...
        // --- Temperatura Água (Atualizado V1.8) ---
        bool wA;
        if (s.ads_fault_active) {
            strcpy(buf,"ADS");
            wA = true;
        } else {
            // O alarme de superaquecimento é engatilhado a 100°C baseados no valor real compensado
            ...
            switch (s.water_status) {
                ...
                default: 
                    // Fase de aquecimento: Oculta valores baixos/frios da tela
                    if (s.water_temp_c < WATER_TEMP_WARMUP) {
                        strcpy(buf, "FRIO");
                    } else {
                        snprintf(buf,sizeof(buf),"%3.0f",s.water_temp_c);
                    }
                    break;
            }
        }
        ...
        if ((millis()-lastBLE) >= 100) {
            lastBLE = millis();
            // A leitura enviada ao RealDash agora é "s.water_temp_c", que já 
            // contém tanto a calibração do offset (+7°C) quanto a suavização exponencial
...
void setup() {
    Serial.begin(115200);
    Serial.println("[BOOT] Chevette ECU V1.8 - Dossie V21.0");
    ...
    Serial.println("[OK] ADC combustivel em 11dB por pino");
    ...
    if (gADS.begin(ADS_I2C_ADDR) && adsApplyRuntimeConfig()) {
        ...
        gData.ads_ok = false;
        gData.ads_fault_active = true;
        Serial.println("[ERRO] ADS1115 nao encontrado ou falhou no teste de I2C!");
    }
    ...
    Serial.println("[OK] GPS UART1 RX=18 TX=17");
...
#if CHEVETTE_HAS_GPIO_GLITCH_FILTER
    {
        ...
        if (gpio_new_pin_glitch_filter(&cfg, &gRpmGlitchFilter) == ESP_OK &&
            ...
            Serial.println("[WARN] Falha ao habilitar glitch filter HW; fallback para SW");
            gRpmGlitchFilter = nullptr;
        }
    }
#else
    Serial.println("[INFO] API HW Filter off; usando debounce SW");
#endif
    ...
    Serial.println("[BOOT] SO Iniciado.");
}
